...
const $2127f1fec3724162$export$19b172c586f937c9 = (str)=>{
    const getTokens = (start = 0, tokens = [])=>{
        ...
        const elementMatch = strFromStart.match(/^(?:parent|above_sibling)/);
        if (elementMatch) {
            const end13 = start + elementMatch[0].length;
            const value = ()=>element[elementMatch[0]];
            const elementToken = $2127f1fec3724162$var$token($2127f1fec3724162$export$2b7f80a2c1376e05.element, start, end13, value);
            return getTokens(end13, tokens.concat(elementToken));
        }
        const propertyMatch = strFromStart.match(/^[a-zA-Z]\w*/);
        if (propertyMatch) {
            const end14 = start + propertyMatch[0].length;
            const propertyToken = $2127f1fec3724162$var$token($2127f1fec3724162$export$2b7f80a2c1376e05.property, start, end14, propertyMatch[0]);
            return getTokens(end14, tokens.concat(propertyToken));
        }
        const stringMatch = strFromStart.match(/^;.*?(?<!\\);/);
        if (stringMatch) {
            const end15 = start + stringMatch[0].length;
            const stringToken = $2127f1fec3724162$var$token($2127f1fec3724162$export$2b7f80a2c1376e05.string, start, end15, stringMatch[0].slice(1, -1).replace(/\\\;/g, ";"));
            return getTokens(end15, tokens.concat(stringToken));
        }
...
const $c3f51faaa3dc8b2a$export$98e6a39c04603d36 = (element, attribute, nameTokens, valueTokens, debug = false)=>{
    ...
    const getObjectWithProperty = (propertyToken)=>{
        ...
        const obj = element instanceof HTMLCanvasElement || attribute.name === "repeat" || attribute.name === "change" || propName === "above_sibling" || propName === "parent" || propName === "canvas" || propName === "above_siblings_off" ? element : element.parentElement;
        if (propName in obj === false) {
            if (propName in element.pInst) return [
                ()=>element.pInst,
                propName
            ];
            console.error(`On ${element.tagName}'s ${attribute.name}, couldn't find ${propName}`);
        }
        return [
            ()=>obj,
            propName
    ...
    const primary = (tokens)=>{
        ...
        switch(token.kind){
            ...
            case (0, $2127f1fec3724162$export$2b7f80a2c1376e05).element:
                const [afterElement, ...afterNext] = remainder;
                if (afterElement.kind === (0, $2127f1fec3724162$export$2b7f80a2c1376e05).member) return call(token.value, afterElement.value, afterNext);
                return [
                    token.value,
                    remainder
                ];
            ...
            case "[":
                ...
                if (rightSquareBracketIndex < 0) console.error(`On ${element.tagName}'s ${attribute.name}, found a [ left square bracket without matching right square bracket]`);
                return [
            ...
            case "{":
                ...
                if (rightCurlyBracketIndex < 0) console.error(`On ${element.tagName}'s ${attribute.name}, found a { left curly bracket without matching right curly bracket}`);
                return [
        ...
        const getCompareFn = ()=>{
            switch(operator.value){
                ...
                case "greater than":
                    return ()=>left() > right();
                default:
                    console.error(`On ${element.tagName}'s ${attribute.name}, comparison token was found with value '${operator.value}', but this value could not be matched with a function.`);
            }
        };
        return [
            getCompareFn(),
    ...
    const end = (tokens)=>{
        ...
        if (afterExpression.length === 0 || afterExpression.length === 1 && afterExpression[0].kind !== (0, $2127f1fec3724162$export$2b7f80a2c1376e05).end) console.error(`On ${element.tagName}'s ${attribute.name}, reached end of expression without an end token`);
        else if (afterExpression.length > 1) console.error(`On ${element.tagName}'s ${attribute.name}, reached end of expression and found remaining tokens: ${afterExpression.map((t)=>t.value).join(" ")}`);
    ...
    const parseObjectMember = (tokens, getBaseObj = null, getLeftMemberName = null)=>{
        if (tokens.length === 0) return [
            getBaseObj,
            getLeftMemberName
        ];
        const [nextToken, ...afterNext] = tokens;
        if (nextToken.kind === (0, $2127f1fec3724162$export$2b7f80a2c1376e05).element) return parseObjectMember(afterNext, nextToken.value);
        if (nextToken.kind === (0, $2127f1fec3724162$export$2b7f80a2c1376e05).property) {
            if (getBaseObj) return parseObjectMember(afterNext, getBaseObj, ()=>nextToken.value);
            return parseObjectMember(afterNext, ()=>element, ()=>nextToken.value);
        }
        const [getObj, getMemberName, remainder] = computedMember(getBaseObj, getLeftMemberName, afterNext);
        return parseObjectMember(remainder, getObj, getMemberName);
    };
    if (debug) console.log(`%cParsing ${attribute.name} = ${attribute.value}`, "background: lightgreen; padding: 10px; color: black");
    const [getTargetObject, getTargetMemberName] = parseObjectMember(nameTokens);
    const getValue = parseAndAutoEnclose(valueTokens);
    if (debug) console.log(getTargetObject.toString(), getTargetMemberName(), getValue());
    return ()=>getTargetObject()[getTargetMemberName()] = getValue();
};


const $a2ba0be321a8c9b5$export$67beed298888a38b = (element, attribute)=>{
    const nameTokens = (0, $2127f1fec3724162$export$19b172c586f937c9)(attribute.name);
    const valueTokens = (0, $2127f1fec3724162$export$19b172c586f937c9)(attribute.value);
    return (0, $c3f51faaa3dc8b2a$export$98e6a39c04603d36)(element, attribute, nameTokens, valueTokens, true);
};


const $109b9a5ca7a37440$export$a00e2346eabbdbef = (el)=>{
    const name = (0, $a2ba0be321a8c9b5$export$67beed298888a38b)(el, el.attributes.name)();
...
const $79ce0e365a23b6d5$export$a11dc51f2ecd743e = (baseClass)=>class P5Extension extends (0, $dfaf816c8f7968eb$export$df7182d31779a5d2)((0, $03bbbf8eda9a336e$export$f5ddaad6515de8cc)((0, $78e88c2be3b82581$export$a58b47d028f9234f)((0, $16ac526ed636526d$export$b46a7ea7c25c5e6d)((0, $9b76c1f65c019db4$export$ee3de0a7dd131a91)((0, $bd52bf81ac7b5a81$export$9f9bf41e917f10e)((0, $74cf9122b1ef1f14$export$89e97ff02600e0c0)((0, $36d0bcbd88d70227$export$480a8cd27449d6bd)(baseClass)))))))) {
     ...
     */ #setupEvalFn(attr) {
            ...
            const interpretation = (0, $a2ba0be321a8c9b5$export$67beed298888a38b)(this, attr);
            this.#updateFunctions.set(attr.name, interpretation);
